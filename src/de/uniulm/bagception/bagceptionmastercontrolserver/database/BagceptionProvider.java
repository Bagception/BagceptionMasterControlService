...
public class BagceptionProvider extends ContentProvider {
	...
	public Uri insert(Uri uri, ContentValues values) {
		...
		if (URI_MATCHER.match(uri) != ITEM_LIST && URI_MATCHER.match(uri) != CATEGORY_LIST) {
		...
		if (URI_MATCHER.match(uri) == ITEM_LIST) {
			...
			long id = db.insertWithOnConflict(DbSchema.TBL_CATEGORIES, null, values, SQLiteDatabase.CONFLICT_REPLACE);
			return getUriForId(id, uri);
		}
	}
